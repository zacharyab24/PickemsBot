...
import (
 ...
 func getFinalsResults() ([]FinalsMatch, []FinalsMatch, []FinalsMatch) {
	 ...
	 // Pad teams list if there is missing information (this happens when matchups are waiting on other games results)
	 // Definitely a better way of doing this but at this point im over working on this bot
	 if len(teams) == 8 {
		 teams = slices.Insert(teams, 4, "TBD")
		 scores = slices.Insert(scores, 4, "0")
	 }
	 if len(teams) == 9 {
		 teams = slices.Insert(teams, 5, "TBD")
		 scores = slices.Insert(scores, 5, "0")
	 }
	 if len(teams) <= 10 {
		 teams = slices.Insert(teams, 10, "TBD")
		 scores = slices.Insert(scores, 10, "0")
	 }
	 if len(teams) == 11 {
		 teams = slices.Insert(teams, 11, "TBD")
		 scores = slices.Insert(scores, 11, "0")
	 }
	 if len(teams) == 12 {
		 teams = slices.Insert(teams, 12, "TBD")
		 scores = slices.Insert(scores, 12, "0")
	 }
	 if len(teams) == 13 {
		 teams = slices.Insert(teams, 13, "TBD")
		 scores = slices.Insert(scores, 13, "0")
	 }
